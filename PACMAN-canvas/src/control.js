...
export const Ctrl = new class {
	...
	#output() {
		...
		if (Ctrl.usingCheats || spd != 'x1.0') {
			puts(0, h*0, Color.Inf[+(spd != 'x1.0') ], 'Speed'+spd, cfg)
			puts(0, h*1, Color.Inf[+Ctrl.invincible ], 'Invincible',cfg)
			puts(0, h*2, Color.Inf[+Ctrl.showTargets], 'Targets',   cfg)
		}
		if (Ctrl.unrestricted) {
			ctx.translate(T*(Cols-5), T/2)
			puts(0,0, Color.Inf[1], 'Un-\nrestricted', cfg)
		}
		ctx.restore()
	}
	#reset() {
