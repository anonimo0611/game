...
const Dirs = freeze([-1, 1]);

function drawOctpus(ctx, aIdx, {Width,Height,Color}) {	
	ctx.save();
...
function drawCrab(ctx, aIdx, {Width,Height,Color}) {
	ctx.save();
	ctx.strokeStyle = Color;
	ctx.fillStyle   = Color;
	ctx.lineCap     = 'round';
	...
	for (const d of Dirs) {
		...
		ctx.beginPath();
			ctx.moveTo(.4*d, 0);
			ctx.quadraticCurveTo(.45*d, 0, .48*d, -.39*aIdx);
		ctx.stroke();
		ctx.restore();
	}
	// Legs
...
function drawSquid(ctx, aIdx, {Width,Height,Color}) {
	ctx.save();
	ctx.strokeStyle = Color;
	ctx.fillStyle   = Color;
	ctx.lineCap     = 'round';
...
function drawUfo(ctx, aIdx, {Width:w,Height:h,Color}) {
	ctx.save();
	ctx.translate(w/2,h/2);
...
const Canvases      = integers(InvaderType.Max).map(()=> Array(2));
const InvaderColors = integers(InvaderType.Max);
function setup(...Classes) {
	for (const cls of Classes) {
		const type = InvaderType[cls.name];
		const {Width,Height,Color}= new cls;
		InvaderColors[type] = Color;
		for (const cvsIdx of integers(2)) {
			const {cvs,ctx}= canvas2D(null, Width,Height);
